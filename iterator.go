...
// skiplist with iterator. The current state of the iterator can be cloned
...
// Value returns the value at the position.
func (it *Iterator) Value() []byte {
	valOffset, valSize := decodeValue(it.nd.value)
...
// Pre moves to the previous position. If there are no previous nodes, then
// Valid() will false after this call
...
// If the record is not present, the Seek positions the iterator on the
// following node (if exists) and returns false.
...
// SeekPrev searches for the record with the given key. If it is present in
// the skiplist, then SeekForPrev positions the iterator on that record and
// returns true. If the record is not present, the SeekForPrev positions the
...
// Add creates a new key/value record if it does not yet exists and positions the
// iteration on it. If the record already exists, then Add postitions the iterator
